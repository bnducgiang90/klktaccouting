...
public class CateController {
    ...
    @PostMapping("/taikhoan/update-balance")
    public ResponseEntity<?> dmtaikhoan_update_balance(HttpServletRequest request, @RequestBody Map<String, Object> data) {
        try {
            Map<String, Object> user_info = (Map<String, Object>)request.getAttribute("user");
            data.put("user", user_info);
            LOGGER.info("data: {}", data);
            Map<String, Object> rs = this.cateService.dmtaikhoan_update_balance(data);
            Map<String, Object> response = new HashMap<>();
            response.put("errorCode", "000");
            response.put("errorMessage", "");
            response.put("data", rs.get("hdr_id"));
            ...
            LOGGER.error("Controller error: dmtaikhoan_update_balance failed", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(e.getMessage());
        } catch (Exception e) {
            LOGGER.error("Unexpected error: dmtaikhoan_update_balance failed", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("System busy, please try again!!!, Internal server error");
        }
    }

    @PostMapping("/taikhoan-chitiet/get-balance")
    public ResponseEntity<?> dmtaikhoan_chitiet_get_balance(HttpServletRequest request, @RequestBody Map<String, Object> data) {
        Map<String, Object> response = new HashMap<>();
        ...
        try {
            ...
            List<Map<String, Object>> rs = this.cateService.dmtaikhoan_chitiet_get_balance(data);
    ...
    @PostMapping("/update")
    public ResponseEntity<?> update(HttpServletRequest request, @RequestBody Map<String, Object> data) {
        try {
            Map<String, Object> user_info = (Map<String, Object>)request.getAttribute("user");
            data.put("user", user_info);
            LOGGER.info("data: {}", data);
            Map<String, Object> rs = this.cateService.update(data);
            Map<String, Object> response = new HashMap<>();
            response.put("errorCode", "000");
            response.put("errorMessage", "");
            response.put("data", rs.get("update_type"));
            return ResponseEntity.ok(response);
        } catch (ServiceException e) {
            LOGGER.error("Controller error: update failed", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(e.getMessage());
        } catch (Exception e) {
            LOGGER.error("Unexpected error: update failed", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("System busy, please try again!!!, Internal server error");
        }
    }
